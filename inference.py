...
class LHMModel:
    ...
    def __init__(self):
        self.df = pd.read_csv('dpschool2abc.csv', delimiter=";")
        self.df['geometry'] = self.df['geometry'].map(lambda x: (float(i) for i in x.split(", ")))
        self.current = None
        self.nearby_df = None
    ...
    def map_nearby_toilets(self):
        ...
        folium.Marker(location=self.current, icon=folium.Icon(color='green')).add_to(my_map)
